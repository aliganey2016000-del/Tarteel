...
function NavItem({ item, active, onNavigate }) {
  ...
  const handleClick = () => {
    onNavigate()
    if (item.href === '#memorize') {
      window.requestAnimationFrame(() => document.getElementById('memorize')?.scrollIntoView({ behavior: 'smooth', block: 'start' }))
    }
  }
...
export default function ReaderNavigation() {
  ...
  const isActive = item => item.href === '/' ? pathname === '/' : item.href === pathname
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        aria-controls="tarteel-reader-navigation"
        className="fixed left-3 top-3 z-[70] grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white/95 text-slate-700 shadow-lg backdrop-blur lg:hidden"
      >
      ...
      <aside
        id="tarteel-reader-navigation"
        aria-label="Primary navigation"
        className={`fixed inset-y-0 left-0 z-[60] flex w-[min(86vw,292px)] flex-col border-r border-slate-200 bg-white shadow-2xl transition-transform duration-200 lg:w-[272px] lg:translate-x-0 lg:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex h-16 shrink-0 items-center gap-3 border-b border-slate-100 px-5">
          ...
          <div>
            <p className="text-lg font-bold tracking-tight text-slate-900">Tarteel</p>
            <p className="text-[11px] font-medium text-slate-400">Quran learning</p>
          </div>
          <button type="button" onClick={close} aria-label="Close navigation" className="ml-auto grid h-9 w-9 place-items-center rounded-xl text-slate-400 hover:bg-slate-50 lg:hidden">
            <X size={18} />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-3" aria-label="Learning sections">
          ...
          <div className="space-y-1">
            {items.map(item => <NavItem key={item.label} item={item} active={isActive(item)} onNavigate={close} />)}
          </div>
        </nav>

        <div className="border-t border-slate-100 p-4">
          <div className="rounded-2xl bg-emerald-50 p-3">
            <p className="text-xs font-semibold text-emerald-800">Read with focus</p>
            <p className="mt-1 text-[11px] leading-5 text-emerald-700/80">The sidebar stays out of the way while you read, and becomes a drawer on phones.</p>
          </div>
        </div>
      </aside>
    </>
