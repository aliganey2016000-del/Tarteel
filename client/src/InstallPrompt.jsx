...
export default function InstallPrompt() {
  ...
  return <aside className="fixed inset-x-3 bottom-3 z-[80] mx-auto max-w-md rounded-3xl border border-emerald-100 bg-white/95 p-4 shadow-2xl backdrop-blur sm:inset-x-auto sm:right-5 sm:w-[min(100%-2rem,390px)]" role="dialog" aria-label="Install Tarteel">
    <div className="flex items-start gap-3">
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-emerald-700 text-white"><Download size={19}/></span>
      <div className="min-w-0 flex-1">
        <p className="font-bold text-slate-900">Install Tarteel</p>
        <p className="mt-1 text-xs leading-5 text-slate-500">Keep Quran reading, memorization and review one tap away with the app experience.</p>
      </div>
    ...
    <div className="mt-3 flex gap-2">
      <button type="button" onClick={install} disabled={installing} className="flex-1 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-800 disabled:opacity-60">{installing ? 'Opening…' : 'Install app'}</button>
      <button type="button" onClick={dismiss} className="rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-200">Not now</button>
    </div>
  </aside>
}
