...
function AppReader() {
  ...
  useEffect(() => {
    ...
    if (!token) return
    getMe(token).then(setUser).catch(() => { try { localStorage.removeItem(TOKEN_KEY) } catch {} })
  }, [])
  ...
  const toggleBookmark = async () => {
    ...
    const token = readToken()
    try {
      if (current.has(currentAyah.number)) {
        current.delete(currentAyah.number)
        if (token) await removeBookmark(token, currentAyah.number)
      } else {
        current.add(currentAyah.number)
  ...
  const syncBookmarks = async (token) => {
    const data = await listBookmarks(token)
    const remoteIds = data.map((item) => item.ayah.id)
    writeState({ bookmarks: remoteIds })
  }

  const onAuthenticated = async (nextUser, token) => {
    try { localStorage.setItem(TOKEN_KEY, token) } catch {}
    setUser(nextUser)
    await syncBookmarks(token).catch(() => {})
    setAccountOpen(false)
  }

  const logout = () => { try { localStorage.removeItem(TOKEN_KEY) } catch {}; setUser(null); setAccountOpen(false) }

  return <div className="min-h-screen bg-[#f7faf8] text-slate-900">
    <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        <button className="flex items-center gap-2" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Tarteel home"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-700 text-white shadow-lg shadow-emerald-700/20"><BookOpen size={20}/></span><span className="text-xl font-bold tracking-tight">Tarteel</span></button>
        <div className="hidden items-center gap-1 md:flex"><span className="rounded-xl bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-800">Quran Reader</span><a className="rounded-xl px-4 py-2 text-sm font-medium text-slate-500 hover:bg-slate-50" href="#memorize">Memorize</a><a className="rounded-xl px-4 py-2 text-sm font-medium text-slate-500 hover:bg-slate-50" href="#progress">Progress</a></div>
        <div className="flex items-center gap-2">
          <label className="hidden items-center rounded-xl border border-slate-200 bg-white px-3 sm:flex"><Search size={17} className="text-slate-400"/><input aria-label="Search Surahs" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Surah..." className="w-32 bg-transparent px-2 py-2.5 text-sm outline-none lg:w-44"/></label>
          <button className="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-600" onClick={() => setAccountOpen((value) => !value)} aria-label="Account">{user ? <UserRound size={18}/> : <LogIn size={18}/>}</button>
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-10">
      <section className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#075e4a] via-[#0b765d] to-[#0d8a6b] p-6 text-white shadow-xl shadow-emerald-900/10 sm:p-9"><p className="text-sm font-medium text-emerald-100">Read · Listen · Reflect</p><h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">The Quran, one ayah at a time.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-emerald-50/90 sm:text-base">Live Uthmani Arabic, Saheeh International translation and Alafasy recitation, with local caching and optional account sync.</p></section>
...
function AccountPanel({ user, onAuthenticated, onLogout, onClose }) {
  ...
  if (user) return <div className="fixed right-4 top-20 z-40 w-[min(92vw,360px)] rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Account</p><h2 className="mt-1 font-bold">{user.name || user.email}</h2><p className="mt-1 text-xs text-slate-500">Your bookmarks and reading progress sync to Tarteel.</p></div><button onClick={onClose} aria-label="Close"><X size={18}/></button></div><button onClick={onLogout} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold"><LogOut size={16}/> Sign out</button></div>
  ...
  return <div className="fixed right-4 top-20 z-40 w-[min(92vw,380px)] rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Tarteel Account</p><h2 className="mt-1 text-xl font-bold">{mode === 'login' ? 'Sign in' : 'Create account'}</h2></div><button onClick={onClose} aria-label="Close"><X size={18}/></button></div><form onSubmit={submit} className="mt-5 space-y-3">{mode === 'register' && <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name (optional)" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-500"/>}<input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-500"/><input required minLength={8} type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password (8+ characters)" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-500"/>{error && <p role="alert" className="rounded-xl bg-red-50 p-3 text-xs text-red-700">{error}</p>}<button disabled={busy} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><UserRound size={16}/>{busy ? 'Please wait…' : mode === 'login' ? 'Sign in' : 'Create account'}</button></form><button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError('') }} className="mt-4 w-full text-center text-xs font-semibold text-emerald-700">{mode === 'login' ? 'New to Tarteel? Create an account' : 'Already have an account? Sign in'}</button><p className="mt-3 text-center text-[11px] leading-4 text-slate-400">Your password is sent only to the configured Tarteel API over HTTPS in production.</p></div>
}
