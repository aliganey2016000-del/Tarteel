...
const FONT_STEP = 4
const FONT_STORAGE_KEY = 'tarteel:single-ayah-font-size:v4'
...
function paginateAyahText(text, measure, availableHeight) {
  const words = splitIntoWords(text)
  if (!words.length || !measure || !Number.isFinite(availableHeight) || availableHeight <= 0) return [String(text || '')]
  ...
  while (start < words.length) {
    ...
    // Greedy pagination keeps complete Arabic words together and lets the browser
    // handle natural Arabic line wrapping. Quran text is small enough for this
    // bounded measurement loop to remain inexpensive even on mobile devices.
    for (let end = start + 1; end <= words.length; end += 1) {
      const candidate = words.slice(start, end).join(' ')
      measure.textContent = candidate
      if (measure.scrollHeight <= availableHeight + 1) {
        bestEnd = end
      } else {
        break
      }
    ...
    pages.push(words.slice(start, bestEnd).join(' '))
    start = bestEnd
  }
...
export default function SingleAyahMode({
  ...
  const [pages, setPages] = useState(() => [String(ayah?.textArabic || '')])
  ...
  useEffect(() => {
    ...
    const observer = typeof ResizeObserver !== 'undefined'
      ...
      : null
    if (readingRef.current && observer) observer.observe(readingRef.current)
  ...
  // Reflow the current ayah whenever its font, line spacing, orientation or
  // available screen area changes. Pagination never changes the Quran data;
  // it only creates reading pages inside the selected ayah.
  useEffect(() => {
    const measure = measureRef.current
    if (!measure || !viewport.width || !viewport.height) return

    const frame = window.requestAnimationFrame(() => {
      const availableHeight = Math.max(120, viewport.height - VIEWPORT_GAP * 2)
      const nextPages = paginateAyahText(ayah?.textArabic, measure, availableHeight)
      setPages(nextPages)
      setPageIndex(current => Math.min(current, Math.max(0, nextPages.length - 1)))
    })

    return () => window.cancelAnimationFrame(frame)
  }, [ayah?.textArabic, fontSize, arabicFontFamily, lineSpacing, viewport.width, viewport.height])
  ...
  }, [pageIndex, pages.length, onPlay, hasNext, hasPrevious])
  ...
  const currentPage = pages[pageIndex] || ayah?.textArabic || ''
  const isFirstPage = pageIndex === 0
  const isLastPage = pageIndex >= pages.length - 1
  const hasInternalPrevious = pageIndex > 0
  const hasInternalNext = pageIndex < pages.length - 1
  ...
  >
    ...
    <header className={`absolute inset-x-0 top-0 z-20 flex items-center justify-between px-4 pt-[max(.7rem,env(safe-area-inset-top))] transition-all duration-300 sm:px-7 ${controlsVisible ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'}`}>
      <button type="button" onClick={onExit} className={`flex h-11 items-center gap-2 rounded-full border px-4 text-sm font-semibold shadow-sm backdrop-blur-xl ${glass}`} aria-label="Exit single Ayah mode">
        <ArrowLeft size={18} />
      ...
      <button type="button" onClick={() => { setShowSize(value => !value); setShowAudio(false); revealControls() }} className={`grid h-11 w-11 place-items-center rounded-full border shadow-sm backdrop-blur-xl ${glass}`} aria-label="Reading controls" title="Reading controls">
        <Settings2 size={18} />
      </button>
    </header>

    <section ref={readingRef} className="relative flex min-h-0 flex-1 items-center justify-center px-4 py-20 sm:px-8 sm:py-24">
      <div className="flex h-full w-full max-w-[1500px] flex-col items-center justify-center">
        <p
          dir="rtl"
          lang="ar"
          className={`w-full px-2 text-center font-medium antialiased transition-all duration-300 ${playing ? 'drop-shadow-[0_0_24px_rgba(16,185,129,.08)]' : ''}`}
          style={{
          ...
          aria-label={`Ayah ${ayah?.numberInSurah}, reading page ${pageIndex + 1} of ${pages.length}`}
        >
          {currentPage}
        </p>

        {showTranslation && translationVisible && <p dir="ltr" className={`mt-7 max-w-3xl px-5 text-center leading-relaxed ${muted}`} style={{ fontSize: `clamp(14px, 2.2vw, ${Math.min(Number(translationSize) || 16, 24)}px)` }}>
          {ayah?.translation}
        </p>}
      </div>
    </section>

    {/* Invisible measurement surface: it uses the exact Arabic font metrics to
        decide where a page can end, so words are never arbitrarily clipped. */}
    <div aria-hidden="true" className="pointer-events-none absolute -left-[100000px] top-0 opacity-0">
      <div
        ref={measureRef}
        dir="rtl"
        lang="ar"
        className="whitespace-normal break-normal text-center font-medium antialiased"
        style={{
          width: `${Math.max(1, viewport.width - 32)}px`,
          fontFamily: arabicFontFamily,
          fontSize: `clamp(${Math.min(fontSize, 56)}px, ${Math.max(7, Math.min(14, fontSize / 6.8))}vw, ${fontSize}px)`,
          lineHeight: lineSpacing,
          padding: '0 8px'
    ...
    {showSize && controlsVisible && <div className={`absolute right-4 top-[calc(max(.7rem,env(safe-area-inset-top))+3.8rem)] z-30 w-[min(92vw,340px)] rounded-3xl border p-4 shadow-2xl backdrop-blur-2xl ${glass}`} onPointerDown={event => event.stopPropagation()} onTouchStart={event => event.stopPropagation()}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`text-[11px] ${muted}`}>Adaptive pages · Arabic words stay intact</p>
        </div>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <input type="range" min={MIN_FONT_SIZE} max={MAX_FONT_SIZE} step={FONT_STEP} value={fontSize} onChange={event => { setFontSize(Number(event.target.value)); revealControls() }} className="h-2 min-w-0 flex-1 accent-emerald-600" aria-label="Arabic text size" />
      ...
      <div className="mt-2 flex items-center justify-between text-[11px] font-semibold tabular-nums"><span>A−</span><span>{fontSize}px</span><span>A+</span></div>
      <div className={`mt-3 rounded-xl px-3 py-2 text-[11px] ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
        Page {pageIndex + 1} of {pages.length} · adjusts automatically to screen
      </div>
    </div>}
    ...
    {showAudio && controlsVisible && <div className={`absolute bottom-[calc(max(1rem,env(safe-area-inset-bottom))+6.4rem)] left-1/2 z-30 w-[min(94vw,440px)] -translate-x-1/2 rounded-3xl border p-4 shadow-2xl backdrop-blur-2xl ${glass}`} onPointerDown={event => event.stopPropagation()} onTouchStart={event => event.stopPropagation()}>
      <div className="flex items-center justify-between gap-3">
        <div><p className="text-sm font-bold">{reciter || 'Reciter'}</p><p className={`text-[11px] ${muted}`}>Playback speed · {speed || 1}×</p></div>
        <Volume2 size={17} className={muted}/>
      </div>
      <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200/50"><div className="h-full rounded-full bg-emerald-500 transition-[width]" style={{ width: `${progress}%` }} /></div>
    ...
    <div className={`absolute inset-x-0 bottom-0 z-20 flex flex-col items-center px-4 pb-[max(.8rem,env(safe-area-inset-bottom))] transition-all duration-300 sm:px-7 ${controlsVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
      <div className="flex w-full max-w-[760px] items-center justify-between gap-2">
        <button type="button" onClick={goPrevious} disabled={isFirstPage && !hasPrevious} className={`h-10 rounded-full px-4 text-xs font-semibold transition disabled:invisible ${glass} border backdrop-blur-xl`} aria-label="Previous page or Ayah">Previous</button>
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={goNext} disabled={isLastPage && !hasNext} className={`h-10 rounded-full px-4 text-xs font-semibold transition disabled:invisible ${glass} border backdrop-blur-xl`} aria-label="Next page or Ayah">Next</button>
        </div>
      ...
      <div className={`mt-3 flex items-center gap-2 transition-opacity duration-300 ${pressed ? 'opacity-100' : 'opacity-90'}`}>
        <button type="button" onClick={() => { setShowTranslation(value => !value); revealControls() }} disabled={!translationVisible} className={`rounded-full border px-3 py-1.5 text-[11px] font-semibold backdrop-blur-xl disabled:opacity-30 ${glass}`} aria-label={showTranslation ? 'Hide translation' : 'Show translation'}>
          {showTranslation ? <EyeOff size={13} className="mr-1 inline"/> : <Eye size={13} className="mr-1 inline"/>}{showTranslation ? 'Hide translation' : 'Translation'}
        </button>
