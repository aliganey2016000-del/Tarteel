...
function ProgressBadge() {
  ...
  useEffect(() => { let cancelled = false; const load = () => { const token = localStorage.getItem(TOKEN_KEY); if (!token) { setSummary(null); return }; getStreaks(token).then(data => { if (!cancelled) setSummary(summarizeStreak(data)) }).catch(() => { if (!cancelled) setSummary(null) }) }; load(); const interval = window.setInterval(load, 60_000); window.addEventListener('storage', load); return () => { cancelled = true; window.clearInterval(interval); window.removeEventListener('storage', load) } }, [])
